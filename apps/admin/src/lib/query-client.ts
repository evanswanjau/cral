...
/**
 * The console's single QueryClient, kept in its own module rather than in
 * `main.tsx` so non-React code can reach it without importing the app
 * entry point (which would be a cycle).
 *
 * `lib/auth.ts` clears it on every session change - cached query data
 * belongs to whichever reviewer was signed in a moment ago and must not
 * survive a sign-out. See `setSession`.
 */
