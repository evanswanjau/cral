 ...
 *    on: nothing pays in instalments yet (no rail). Default `weekly`; the
 *    service coerces it to `monthly` for bank payouts.
 *  - `payout_mpesa_name` — "Name on the M-Pesa line" from the design.
 *    Not collected at onboarding (which only takes the number); nullable.
 *
 * The M-Pesa *number* is not a stored field — it is always `users.phone`
 * (to change it you change your phone on the profile). The bank columns
 * and `payout_method` already exist from the original create-merchants
 * migration.
 */
